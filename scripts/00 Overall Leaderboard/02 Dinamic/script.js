...
function initPuntosEquipoLiga(config) {
    ...
    const PAGE_CYCLE_INTERVAL = parseInt(config.Settings.n_LigaPageCycleInterval || '30000', 10);
    ...
    const createTeamItemHTML = (teamData) => {
        ...
        let mapScoresHTML = '';
        for (let i = 1; i <= mapsPlayed; i++) { mapScoresHTML += `<div class="map-score">${teamData[`Map${i}`] || '-'}</div>`; }
        ...
        // ESTRUCTURA CORREGIDA: team-identity separada de map-scores
        return `<div class="${boxClass}" data-team-name="${cleanName}">
                    ...
                    <div class="team-info-container">
                        ...
                        <div class="map-scores-container">${mapScoresHTML}</div>
                    </div>
    ...
    const updateTeamItemDOM = (domElement, teamData) => {
        try {
            ...
            if (elMapContainer) {
                let mapScoresHTML = '';
                for (let i = 1; i <= mapsPlayed; i++) { mapScoresHTML += `<div class="map-score">${teamData[`Map${i}`] || '-'}</div>`; }
                elMapContainer.innerHTML = mapScoresHTML;
            }

            // Estilos de Grupo (Limpieza mejorada)
            let boxClass = "item-box";
            if (enableQualification && parseInt(Top, 10) <= qualificationThreshold) { boxClass += " qualification-highlight"; }
            ...
            if (showLogos && logosPath) {
                ...
                if (newCleanName !== currentCleanName) {
                    ...
                    if (elLogoContainer) {
                        ...
                        // Si no existía logo, lo ponemos al principio de la identidad
    ...
    const displayCurrentPage = () => {
        ...
        // Comparamos longitudes para decidir si recrear o actualizar
        ...
        // Re-aplicar grid maps por seguridad
        gridContainer.querySelectorAll('.map-scores-container').forEach(container => {
            container.style.gridTemplateColumns = `repeat(${mapsPlayed}, 1fr)`;
        });
        ...
        // Actualizamos leyenda al final para asegurar sincronía
        updatePageLegend(currentPage, totalPages, itemsPerPage, allData.length);
    };
    
    const cycleToNextPage = () => { const totalPages = Math.ceil(allData.length / itemsPerPage); currentPage = totalPages > 0 ? (currentPage + 1) % totalPages : 0; displayCurrentPage(); };
    ...
    setInterval(cycleToNextPage, PAGE_CYCLE_INTERVAL);
}