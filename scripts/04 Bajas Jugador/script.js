...
function initBajasJugadorGrid(config) {
    const DATA_REFRESH_INTERVAL = 10000;
    setDynamicBackground(document.body, 'bajas_jugador', config);
    ...
    const offset = parseInt(config.Settings.n_BajasJugadorOffset || '0', 10);
    ...
    // --- HELPER LOGO (Misma lógica que Team Standings) ---
    ...
    const createPlayerItemHTML = (playerData) => {
        const { Top = "-", Jugador = "N/A", Kills = "0", Rondas = "0", Grupo = "", Equipo = "" } = playerData;
        ...
        return `<div class="item-box ${prizeClass}" data-team-name="${cleanTeamName}">
    ...
    const updatePlayerItemDOM = (domElement, playerData) => {
        ...
        domElement.className = `item-box ${prizeClass}`;
        ...
        if (showTeamLogos && logosPath) {
            ...
            if (newCleanTeamName !== currentCleanTeamName) {
                ...
                if (elLogoContainer) {
                    ...
                    // Si no existía (config cambiada o primera carga rara), insertamos después del item-top
    ...
    const displayPage = () => {
        ...
        if (currentItems.length === displayData.length && currentItems.length > 0) {
            ...
            // RENDERIZAR nuevos (primera carga o cambio de número de items)
            let newHTML = "";
