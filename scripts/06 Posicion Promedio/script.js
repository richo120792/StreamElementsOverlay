...
async function main(configUrl) {
    ...
    if (!config) return;
    setupCanvas(config);
    initPosicionPromedioGrid(config);
}
...
function initPosicionPromedioGrid(config) {
    const DATA_REFRESH_INTERVAL = 10000;
    setDynamicBackground(document.body, 'posicion_promedio_equipo', config);
    ...
    const offset = parseInt(config.Settings.n_PosicionPromedioOffset || '0', 10);
    ...
    const createTeamItemHTML = (teamData) => {
        const { Top = "-", Equipo = "N/A", "Top Promedio": topPromedio = "0.0", Rondas = "0", Grupo = "" } = teamData;
        ...
        return `<div class="item-box" data-team-name="${cleanTeamName}">
                    ...
                    ${logoHTML}
                    <div class="item-info"><div class="item-team-name">${Equipo}</div><div class="item-players">${playersHTML}</div></div>
                    <div class="item-points">${topPromedio}</div>
                </div>`;
    };
    ...
    const updateTeamItemDOM = (domElement, teamData) => {
        ...
        if(elPoints) elPoints.textContent = topPromedio;
    ...
    const refreshData = async () => {
        const fetchedData = await fetchSheetDataSE("GID_PosicionPromedioEquipo", config);
        ...
        if (Array.isArray(fetchedData) && fetchedData.length > 0) {
            ...
            lastDataJSON = currentDataJSON; // Actualizar memoria
            // ----------------------------------

            allData = fetchedData;
            displayPage();
        } else {
            console.log("PosicionPromedio: No se recibieron datos nuevos o hubo un error.");
        }
    };
