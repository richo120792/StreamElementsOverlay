...
function initKillsEquipoGrid(config) {
    const DATA_REFRESH_INTERVAL = 10000;
    setDynamicBackground(document.body, 'kills_equipo', config);
    ...
    const createTeamItemHTML = (teamData) => {
        const { Top = "-", Equipo = "N/A", Kills = "0", Rondas = "0", Grupo = "" } = teamData;
        ...
        return `<div class="item-box" data-team-name="${cleanTeamName}">
                    ...
                    <div class="item-points">${Kills}</div>
                </div>`;
    };
    ...
    const updateTeamItemDOM = (domElement, teamData) => {
        ...
        if(elPoints) elPoints.textContent = Kills;
    ...
    const refreshData = async () => {
        const fetchedData = await fetchSheetDataSE("GID_KillsEquipo", config);
        ...
        if (Array.isArray(fetchedData) && fetchedData.length > 0) {
            ...
            console.log("KillsEquipo: No se recibieron datos nuevos o hubo un error.");
        }
    };
